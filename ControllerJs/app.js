...
const alertManager = (typeMsg, message,idToast) => 
{
    let idBodyToast=""
    idToast=="tProductos"? idBodyToast="bPro" : idBodyToast="bCat" ;
    let alert = document.querySelector(`#${idToast}`);
    let bodyToast = document.querySelector(`#${idBodyToast}`);
    bodyToast.innerHTML =` ${message}`  || 'Se produjo cambios';
    alert.classList.remove("hide");
    alert.classList.add(typeMsg,"show");

    setTimeout(() => {
    alert.classList.add("hide"); 
    alert.classList.remove(typeMsg,"show");
    }, 5000);
}

const cerrarToast=(idToast)=>{
    let alertC = document.querySelector(`#${idToast}`);
    alertC.classList.remove("show");
    alertC.classList.add("hide"); 
...
const obtenerProductos=()=>
{
    ...
    .then(response => response.json())//decalaramos que la respuesta del backend sea en formato json para su manipulacion
